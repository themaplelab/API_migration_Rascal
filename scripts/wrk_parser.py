...
def readFile(n, fileName):
    file1 = open("/home/hewage/data/experiments/scripts/Data/" + n + "/" + fileName, "r+")
    return file1.read()


def writeFile(n, data):
    file1 = open("/home/hewage/data/experiments/scripts/Data/" + n + "/original_final_output_" + n +".csv", "w+")
...
def main(n):
    ...
    for i in range(1, 21):
        #for j in range(0,8):
            wrk_output = readFile(n,"output-" + str(i) +  "-9093-1"+ ".txt")
            #wrk_output = readFile(n,"output-" + str(i) + ".txt")
